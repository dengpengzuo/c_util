...
/* 结构定义 */
typedef struct ez_event_loop_s ezEventLoop_t;
...
typedef void (*ezFileProc)(ezEventLoop_t * eventLoop, int fd, void *clientData, int mask);
typedef int (*ezTimeProc)(ezEventLoop_t * eventLoop, int64_t timeId, void *clientData);
...
void ez_delete_time_event(ezEventLoop_t * eventLoop, int64_t time_id);
