...
#define cast_to_type(ptr, type)     ez_container_of(ptr, type, rb_node)
...
{
    student *n = cast_to_type(new_node, student);
    student *o = cast_to_type(exists_node, student);
...
static int student_find_proc(ezRBTreeNode *node, void *find_args)
{
    ...
    student *o = cast_to_type(node, student);
