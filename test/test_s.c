...
static ezWorkerIndex = 0;

void *ez_worker_thread_run(void *t)
{
	ezWorker *worker = (ezWorker*)t;

	log_info("create worker id: %d event loop .", worker->id);

	worker->w_event = ez_create_event_loop(1024);
	ez_run_event_loop(worker->w_event);

	log_info("delete worker id: %d event loop .", worker->id);
	ez_delete_event_loop(worker->w_event);
...
{
	int i = ezWorkerIndex;
	ezWorkerIndex = (++ ezWorkerIndex) & ( WORKER_SIZE-1);

	ez_net_set_non_block(c);
	ez_net_tcp_enable_nodelay(c);
	ez_net_tcp_keepalive(c, 30);

	ezWorker *cworker = &workers[i];
	log_info("worker id:%d add client %d .", cworker->id, c);
	if (ez_create_file_event(cworker->w_event, c, AE_READABLE, read_client_handler, NULL) == AE_ERR) {
		log_info("add eventloop new client %d AE_READABLE failed!", c);
		ez_net_close_socket(c);
	}
}

void init_ez_worker_thread()
{
	for(int i=0;i<WORKER_SIZE;++i)
	{
		workers[i].id = i;
		pthread_create(&(workers[i].thid), NULL, ez_worker_thread_run, &workers[i]);
	}
}

void stop_ez_worker_thread()
{
	for(int i=0;i<WORKER_SIZE;++i)
	{
		log_info("worker id: %d exit event loop .", worker->id);
		ez_stop(workers[i].w_event);
	}
	for(int i=0;i<WORKER_SIZE;++i)
	{
		log_info("waite worker id: %d thread exit.", worker->id);
